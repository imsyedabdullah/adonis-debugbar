...
function MethodBadge({ method }: { method: string }) {
  ...
  return (
    <span style={{
      ...
      fontFamily: 'monospace', background: color + '22', color, flexShrink: 0,
    }}>
      {method}
    </span>
  )
...
function RequestRow({ request, selected, onClick }: {
  ...
  return (
    ...
    >
      ...
      <span style={{
        flex: 1, fontFamily: 'monospace', fontSize: 12, color: s.textPrimary,
        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 0,
      }}>
        {getPath(request.url)}
      </span>
      <StatusBadge status={request.status} />
      <span style={{ fontSize: 11, color: durationColor(request.duration), fontWeight: 600, flexShrink: 0, minWidth: 40, textAlign: 'right' }}>
        {request.duration.toFixed(0)}ms
      </span>
      <span style={{ fontSize: 11, color: s.textMuted, flexShrink: 0, minWidth: 56, textAlign: 'right' }}>
...
function RequestSelector({ requests, selectedUid, onSelect }: {
    ...
    <div ref={wrapperRef} style={{ flex: 1, position: 'relative', minWidth: 0 }}>
      <button
        ...
        style={{
          ...
          border: `1px solid ${s.border}`, borderRadius: open ? '4px 4px 0 0' : 4,
      ...
      >
        {selected ? (
          <>
            ...
            <span style={{ fontSize: 11, color: durationColor(selected.duration), fontWeight: 600, flexShrink: 0 }}>
              {selected.duration.toFixed(0)}ms
            </span>
          </>
        ) : (
          <span style={{ fontSize: 12, color: s.textMuted, textAlign: 'left' }}>No requests captured yet</span>
        )}
        <span style={{
          marginLeft: 4, color: s.textMuted, fontSize: 10, flexShrink: 0,
        ...
        <div style={{
          ...
          {requests.map((r) => (
            <RequestRow
              ...
              onClick={() => { onSelect(r.uid); setOpen(false) }}
            />
          ))}
        </div>
      )}
...
export function Debugbar({ baseUrl = '' }: Props) {
  ...
  useEffect(() => {
    ...
    const onSuccess = (e: Event) => {
      try { readPage((e as CustomEvent).detail?.page ?? {}) } catch {}
    }
  ...
  useEffect(() => {
    if (requests.length === 0) { setSelectedUid(null); return }
    setSelectedUid((prev) => {
  ...
  const tabs: { key: Tab; label: string; badge?: number; badgeColor?: string }[] = [
    ...
    { key: 'session',    label: 'Session',    badge: data?.session ? Object.keys(data.session.data).length : undefined },
  ]
  ...
  return (
    <div style={{
      ...
      {open && (
        <>
          <div
            ...
            style={{
              height: HANDLE_HEIGHT, cursor: 'ns-resize',
              background: s.border, transition: 'background 0.15s',
            }}
          ...
          <div style={{
            ...
            <div style={{
              ...
              padding: '5px 12px', borderBottom: `1px solid ${s.border}`,
            ...
            <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, background: s.bg }}>
              {loading && (
                <div style={{ padding: '24px 16px', color: s.textMuted, fontSize: 13, textAlign: 'center' }}>
                  Loading…
                </div>
              )}
              {!loading && !data && (
                <div style={{ padding: '24px 16px', color: s.textMuted, fontSize: 13, textAlign: 'center' }}>
                  {requests.length === 0 ? 'Make a request to see debug data.' : 'Select a request above.'}
                </div>
              )}
              {!loading && data && activeTab === 'messages'   && <MessagesPanel messages={data.messages} />}
              {!loading && activeTab === 'console'    && <ConsolePanel logs={data?.consoleLogs ?? []} />}
              ...
              {!loading && data && activeTab === 'session'    && <SessionPanel session={data.session ?? null} />}
            </div>
          </div>
        </>
      )}
      ...
      <div style={{
          ...
          return (
            ...
            >
              ...
              {tab.badge !== undefined && tab.badge > 0 && (
                <span style={{
                  background: isActive ? accentColor : (tab.badgeColor ? tab.badgeColor + '22' : s.border),
        ...
        <div style={{ flex: 1 }} />
        {open && requests.length > 0 && (
          <button onClick={clear} style={actionBtn} title="Clear all requests">Clear</button>
        )}
        {open && (
          <button
