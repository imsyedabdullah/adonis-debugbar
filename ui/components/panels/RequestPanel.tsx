  ...
  return (
    <div>
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${s.border}`, fontSize: 13,
      }}>
        <span style={{
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ borderBottom: `1px solid ${s.border}` }}>
      <div style={{
        padding: '6px 16px', background: s.surface,
        fontSize: 11, fontWeight: 600,
        color: s.textSecondary, textTransform: 'uppercase', letterSpacing: 0.7,
      ...
      <tbody>
        {entries.map(([k, v]) => (
          <tr key={k} style={{ borderBottom: `1px solid ${s.border}` }}>
            <td style={{ padding: '6px 16px', color: s.blue, fontFamily: 'monospace', whiteSpace: 'nowrap', width: '35%', verticalAlign: 'top' }}>
              {k}
            </td>
            <td style={{ padding: '6px 16px', color: s.textPrimary, fontFamily: 'monospace', wordBreak: 'break-all' }}>
              {mask?.has(k.toLowerCase()) ? <span style={{ color: s.textMuted }}>•••••••</span> : String(v)}
            </td>
          </tr>
        ))}
      </tbody>
...
const codeBlock: React.CSSProperties = {
  margin: '8px 16px', fontFamily: 'monospace', fontSize: 12,
  background: s.codeBg, padding: '10px 14px',
  borderRadius: 6, border: `1px solid ${s.border}`,
  whiteSpace: 'pre-wrap', wordBreak: 'break-word',
  color: s.codeColor, lineHeight: 1.6, maxHeight: 200, overflowY: 'auto',
}

const toggleBtn: React.CSSProperties = {
  margin: '8px 16px 4px',
  fontSize: 12, color: s.blue,
