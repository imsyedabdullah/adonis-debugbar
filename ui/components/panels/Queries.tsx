...
function formatSql(sql: string): string {
  return sql
    .replace(
      /\b(SELECT|FROM|WHERE|JOIN|LEFT JOIN|RIGHT JOIN|INNER JOIN|AND|OR|ORDER BY|GROUP BY|HAVING|LIMIT|OFFSET|INSERT INTO|VALUES|UPDATE|SET|DELETE FROM|RETURNING|WITH|UNION)\b/gi,
      (m) => `\n${m.toUpperCase()}`,
    )
    .trim();
}
...
function classifyLine(line: string): { bg: string; scanColor: string | null } {
  ...
  if (/Buffers:/i.test(line)) return { bg: s.purple + '08', scanColor: null };
...
function PlanLineContent({ line }: { line: string }) {
  for (const [re, color] of SCAN_COLORS) {
    if (re.test(line)) {
      ...
      while ((m = g.exec(line)) !== null) {
        ...
        parts.push(
          <span key={m.index} style={{ color, fontWeight: 700 }}>
            {m[0]}
          </span>,
        );
  ...
  const NUM =
    /((?:cost|actual time|rows|width|Execution Time|Planning Time)\s*=?\s*)([\d.]+(?:\.\.\d+)?)/gi;
  const parts: React.ReactNode[] = [];
  let pos = 0;
  let m: RegExpExecArray | null;
  while ((m = NUM.exec(line)) !== null) {
    if (m.index > pos) parts.push(line.slice(pos, m.index));
    parts.push(
      <span key={m.index} style={{ color: s.textSecondary }}>
        {m[1]}
      </span>,
    );
    parts.push(
      <span key={m.index + 'v'} style={{ color: s.blue, fontWeight: 600 }}>
        {m[2]}
      </span>,
    );
    pos = m.index + m[0].length;
      ...
      <div style={{ overflowX: 'auto' }}>
        {lines.map((line, i) => {
          ...
          return (
            ...
            >
              {' '.repeat(indent)}
              <PlanLineContent line={line.trimStart()} />
            </div>
          );
        })}
      </div>
            ...
            {rows.map((row, i) => (
              ...
              >
                {cols.map((c) => {
                  ...
                  const color =
                    c === 'type'
                      ? (MYSQL_TYPE_COLORS[val] ?? s.textPrimary)
                      : c === 'Extra'
                        ? s.textSecondary
                        : s.textPrimary;
                  return (
...
function ExplainOutput({ result }: { result: ExplainResult }) {
  ...
  return (
    <>
      ...
      {(result.dbClient === 'mysql' || result.dbClient === 'mysql2') &&
        Array.isArray(result.plan) && (
          <ExplainTableMysql
            rows={result.plan as Record<string, unknown>[]}
            timings={result.timings}
          />
        )}
      {(result.dbClient === 'sqlite3' ||
        result.dbClient === 'better-sqlite3' ||
        result.dbClient === 'unknown') &&
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button
            ...
            style={{
              ...
              padding: '2px 8px',
              background:
                copyExplainState === 'copied'
                  ? s.green + '22'
                  : copyExplainState === 'error'
                    ? s.red + '22'
                    : s.surface,
              border: `1px solid ${
                copyExplainState === 'copied'
                ...
                >
                  ...
                  <td
                    style={{
                      ...tdStyle,
                      fontFamily: 'monospace',
                      color: q.error ? s.red : s.textPrimary,
                    }}
                  >
                    <div
                      style={{
                  ...
                  <td
                    style={{
                      ...
                      textAlign: 'right',
                      fontWeight: 600,
                      color: durationColor(q.duration),
                    }}
                  >
                    ...
                    <td colSpan={2} style={{ padding: '12px 16px' }}>
                      ...
                      <div style={{ marginTop: 10 }}>
                        ...
                        {explainState.status === 'loading' && (
                          <span
                            style={{ fontSize: 11, color: s.textMuted, fontFamily: 'monospace' }}
                          >
                            Running EXPLAIN…
                          </span>
                        )}
