...
const tdStyle: React.CSSProperties = { padding: '7px 12px', fontSize: 12, color: s.textPrimary, verticalAlign: 'middle' }
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{
        ...
        fontSize: 10, fontWeight: 600, color: s.textSecondary,
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        <thead>
          <tr style={{ background: s.surface, borderBottom: `1px solid ${s.border}` }}>
            <Th w={180}>Label</Th>
            <Th>Bar</Th>
            <Th w={80} right>Offset</Th>
            <Th w={80} right>Duration</Th>
          </tr>
        </thead>
        <tbody>
          {measures.map((m, i) => {
            ...
            return (
              <tr key={i} style={{ borderBottom: `1px solid ${s.border}` }}>
                <td style={tdStyle}>
                  ...
                  {incomplete && (
                    <span style={{
                      ...
                      color: s.yellow, background: s.yellow + '22',
                ...
                <td style={{ ...tdStyle, padding: '8px 12px' }}>
                  <div style={{ position: 'relative', height: 14, background: s.surface, borderRadius: 3, overflow: 'hidden', border: `1px solid ${s.border}` }}>
                    <div style={{
                      position: 'absolute',
                      left: `${barLeft}%`,
                      width: `${barWidth}%`,
                ...
                <td style={{ ...tdStyle, textAlign: 'right', color: s.textSecondary, fontFamily: 'monospace' }}>
                  +{m.startMs.toFixed(2)}ms
                </td>
                <td style={{ ...tdStyle, textAlign: 'right', fontFamily: 'monospace', color: incomplete ? s.yellow : s.textSecondary }}>
...
interface Props { data: DebugbarData }
    ...
    <div>
      ...
      <div style={{
        ...
        <span>
          ...
          &nbsp;·&nbsp;
          <strong style={{ color: s.textPrimary }}>{queryCount}</strong> {queryCount === 1 ? 'query' : 'queries'} ({queryTime.toFixed(2)} ms)
          {measures.length > 0 && (
            <>&nbsp;·&nbsp;<strong style={{ color: s.purple }}>{measures.length}</strong> measure{measures.length !== 1 ? 's' : ''}</>
          )}
        </span>
      ...
      {!markers.length && !measures.length && (
        <div style={{ marginBottom: 12, fontSize: 11, color: s.textMuted }}>
          {[
            ...
            { label: "Debugbar.startMeasure('label') / stopMeasure('label')", desc: 'measure a code block' },
          ].map(({ label, desc }) => (
            <div key={label} style={{ marginBottom: 4 }}>
              <code style={{ fontFamily: 'monospace', background: s.codeBg, padding: '1px 4px', borderRadius: 3, color: s.codeColor }}>
                {label}
              </code>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, marginBottom: 8 }}>
        <thead>
          <tr style={{ background: s.surface, borderBottom: `1px solid ${s.border}` }}>
            <Th w={180}>Event</Th>
            <Th>Bar</Th>
            <Th w={90} right>Time (ms)</Th>
            <Th w={90} right>+Delta</Th>
          </tr>
        </thead>
        <tbody>
          {entries.map((entry, i) => {
            ...
            return (
              <tr key={i} style={{ borderBottom: `1px solid ${s.border}` }}>
                ...
                <td style={{ ...tdStyle, padding: '8px 12px' }}>
                  <div style={{ position: 'relative', height: 14, background: s.surface, borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{
                      position: 'absolute', left: `${barLeft}%`, width: `${barWidth}%`,
                      height: '100%', background: i === 0 ? s.blue : s.green,
                      borderRadius: 2, minWidth: 3,
                    }} />
                  </div>
                </td>
                <td style={{ ...tdStyle, textAlign: 'right', color: s.textSecondary }}>{entry.ms.toFixed(2)}</td>
                <td style={{ ...tdStyle, textAlign: 'right', color: s.textSecondary }}>+{segMs.toFixed(2)}</td>
              </tr>
            )
          })}
          {markers.length > 0 && (
            <tr style={{ borderBottom: `1px solid ${s.border}` }}>
              ...
              <td style={{ ...tdStyle, padding: '10px 12px 0px' }}>
                ...
                <div style={{ position: 'relative' }}>
                  {/* the gray bar */}
                  <div style={{
                    height: 14, background: s.borderStrong,
