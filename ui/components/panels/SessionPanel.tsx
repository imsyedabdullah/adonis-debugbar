    ...
    return (
      <span>
        <button onClick={() => setOpen(!open)} style={{
          ...
          {open ? '▾' : '▸'} {!Array.isArray(value) ? `{${Object.keys(value as object).length}}` : `[${(value as unknown[]).length}]`}
        </button>
        {!open && isLong && <span style={{ fontFamily: 'monospace', fontSize: 11, color: s.textSecondary }}>{preview.slice(0, 60)}…</span>}
        {!open && !isLong && <span style={{ fontFamily: 'monospace', fontSize: 11, color: s.textSecondary }}>{preview}</span>}
        {open && (
          <pre style={{
  ...
  if (str.length > 80) {
    return (
      <span style={{ fontFamily: 'monospace', fontSize: 12, color: s.textPrimary }}>
        {str.slice(0, 80)}<span style={{ color: s.textMuted }}>…({str.length} chars)</span>
      </span>
    )
  }
...
function MetaChip({ label, value, mono = false }: { label: string; value: string; mono?: boolean }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
      <span style={{ fontSize: 10, fontWeight: 600, color: s.textMuted, textTransform: 'uppercase', letterSpacing: 0.5 }}>
        {label}
      </span>
      <span style={{
        fontSize: 11, fontFamily: mono ? 'monospace' : 'inherit',
        color: s.textPrimary, background: s.codeBg,
        padding: '1px 6px', borderRadius: 3, border: `1px solid ${s.border}`,
      }}>
        {value}
      </span>
    </div>
...
function Badge({ color, children }: { color: string; children: React.ReactNode }) {
  return (
    <span style={{
      ...
      fontWeight: 600, background: color + '1a', color,
          ...
          <tbody>
            {entries.map(([k, v], i) => (
              <tr key={k} style={{
                ...
                <td style={{ ...tdStyle, fontFamily: 'monospace', color: s.green, whiteSpace: 'nowrap', width: '35%', verticalAlign: 'top' }}>
                  {k}
                  {keyBadge(k)}
                </td>
