...
function safeArg(v: unknown): unknown {
  ...
  try { JSON.stringify(v); return v } catch { return `[non-serializable: ${Object.prototype.toString.call(v)}]` }
}
...
function patchConsole(level: LogLevel, method: 'log' | 'warn' | 'error'): () => void {
  ...
  console[method] = (...args: unknown[]) => {
    ...
    if (entry) entry.consoleLogs.push({ level, args: args.map(safeArg), timestamp: Date.now(), source: 'backend', location: captureCallSite() })
  }
